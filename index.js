...
function jokeApiCall() { 
    ...
    fetch("https://api.api-ninjas.com/v1/jokes?limit=1", { // add event request url
        headers: { 'X-Api-Key': 'JFXRiOWn2ELtxCKG9QXPSw==BjL6EMzuSHq6qrmZ' } // add api key
    })
...
function dadJokeApiCall() {

    dadJokeText.textContent = "Fetching a dad joke...";

    fetch("https://api.api-ninjas.com/v1/dadjokes?limit=1", {
        headers: { 'X-Api-Key': 'JFXRiOWn2ELtxCKG9QXPSw==BjL6EMzuSHq6qrmZ' }
    })
    ...
    .then(function(data) {
        dadJokeText.textContent = data[0].joke;
    })
...
getJokeBtn.addEventListener("click", jokeApiCall)

getDadJokeBtn.addEventListener("click", dadJokeApiCall)
